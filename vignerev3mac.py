...
# Helper function to get a single keypress on macOS
def getch():
    fd = sys.stdin.fileno()
...
# Helper function to check if character is special
SPECIAL_KEYS = {
    '\x1b': 'esc',
    '\x7f': 'backspace',
    ' ': 'space'
...
def encoutput(key):
    from string import ascii_uppercase as alphabetlist

    for item in key.upper():
        if item not in alphabetlist:
            return -3

    keystring, _, _, _ = generatingkeystring(key, 'x' * 2000)
    keystringcounter1 = 0
    string2a = ''
    string2b = ''
    string2c = ''
    ...
    while True:
        ...
        elif key_name == 'backspace' and string2a:
            if string2a[-1] != ' ':
                keystringcounter1 -= 1
            string2a = string2a[:-1]
            string2b = string2b[:-1]
        ...
        elif key_name.upper() in alphabetlist:
            cipher = encrypting(keystring[keystringcounter1], key_name.upper())
            string2a += key_name.upper()
            string2b += cipher
            string2c += keystring[keystringcounter1]
            keystringcounter1 += 1
            encscreen()
